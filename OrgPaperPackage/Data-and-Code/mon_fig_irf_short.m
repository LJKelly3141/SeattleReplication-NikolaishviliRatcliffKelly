...
shortIRFcredset = prctile(shortIRF,[16 50 84],1)/median(shortIRF(:,1,1))*0.25;
...
for j = 1:n
    ...
    d_short = (shortIRFcredset(1,1:H,j))';  
    d_base = (baseIRFcredset(1,1:H,j))';  
    m_short  = (shortIRFcredset(2,1:H,j))';  
    m_base  = (baseIRFcredset(2,1:H,j))';  
    u_short = (shortIRFcredset(3,1:H,j))';  
    ...
    f_short = fill([1:H fliplr(1:H)],[u_short' fliplr(d_short')],[1 0.5 0],'EdgeColor',[1 0.5 0]);
    f_short.FaceAlpha = 0.25;
    plot([1:H],(m_short),'Color',[1 0.5 0],'LineStyle','-','LineWidth',0.75,'HandleVisibility','off');
...
print('-fillpage','-dpdf','mon_fig_irf_short')
