...
vnames = {'\fontsize{11}\rm EONIA'    '\fontsize{11}\rm Industrial production' '\fontsize{11}\rm Core consumer prices' '\fontsize{11}\rm Corp. bond spread' '\fontsize{11}\rm M1'         '\fontsize{11}\rm EUR/USD'    ;
...
coreIRFcredset = prctile(coreIRF,[16 50 84],1)/median(coreIRF(:,1,1))*0.25;
...
for j = 1:n
    ...
    d_core = (coreIRFcredset(1,1:H,j))';  
    d_base = (baseIRFcredset(1,1:H,j))';  
    m_core  = (coreIRFcredset(2,1:H,j))';  
    m_base  = (baseIRFcredset(2,1:H,j))';  
    u_core = (coreIRFcredset(3,1:H,j))';  
    ...
    f_core = fill([1:H fliplr(1:H)],[u_core' fliplr(d_core')],[1 0.5 0],'EdgeColor',[1 0.5 0]);
    f_core.FaceAlpha = 0.25;
    plot([1:H],(m_core),'Color',[1 0.5 0],'LineStyle','-','LineWidth',0.75,'HandleVisibility','off');
...
print('-fillpage','-dpdf','mon_fig_irf_core')
