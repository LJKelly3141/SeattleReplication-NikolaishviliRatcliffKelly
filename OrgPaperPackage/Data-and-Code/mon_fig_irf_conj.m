...
conjIRFcredset = prctile(conjIRF,[16 50 84],1)/median(conjIRF(:,1,1))*0.25;
...
for j = 1:n
    ...
    d_conj = (conjIRFcredset(1,1:H,j))';  
    d_base = (baseIRFcredset(1,1:H,j))';  
    m_conj  = (conjIRFcredset(2,1:H,j))';  
    m_base  = (baseIRFcredset(2,1:H,j))';  
    u_conj = (conjIRFcredset(3,1:H,j))';  
    ...
    f_conj = fill([1:H fliplr(1:H)],[u_conj' fliplr(d_conj')],[1 0.5 0],'EdgeColor',[1 0.5 0]);
    f_conj.FaceAlpha = 0.25;
    plot([1:H],(m_conj),'Color',[1 0.5 0],'LineStyle','-','LineWidth',0.75,'HandleVisibility','off');
...
print('-fillpage','-dpdf','mon_fig_irf_conj')
