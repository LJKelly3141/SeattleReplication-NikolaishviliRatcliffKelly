...
furIRFcredset = prctile(furIRF,[16 50 84],1)/median(furIRF(:,1,1))*0.25;
...
for j = 1:n
    ...
    d_fur = (furIRFcredset(1,1:H,j))';  
    d_base = (baseIRFcredset(1,1:H,j))';  
    m_fur  = (furIRFcredset(2,1:H,j))';  
    m_base  = (baseIRFcredset(2,1:H,j))';  
    u_fur = (furIRFcredset(3,1:H,j))';  
    ...
    f_fur = fill([1:H fliplr(1:H)],[u_fur' fliplr(d_fur')],[1 0.5 0],'EdgeColor',[1 0.5 0]);
    f_fur.FaceAlpha = 0.25;
    plot([1:H],(m_fur),'Color',[1 0.5 0],'LineStyle','-','LineWidth',0.75,'HandleVisibility','off');
...
print('-fillpage','-dpdf','mon_fig_irf_fur')
